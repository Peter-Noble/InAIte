...
def boundingSphere(pos, radius, objectName):
    dim = (radius, radius, radius)
    return BoundingBox(pos, dim, objectName, isSphere=True)
...
def createOctree(boundingBoxes):
    """Make an octree from a bounding box"""
...
class Leaf:
    ...
    def printTree(self, depth=0):
        print(depth*"--", self.name, [c.original for c in self.contents])
...
if __name__ == "__main__":
    ...
    bbs = []
    for ob in bpy.context.scene.objects:
        bbs.append(boundingSphere(ob.location, ob.dimensions[0]/2, ob.name))
