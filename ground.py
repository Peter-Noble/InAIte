...
def intersection(from_obj, to_obj):
    """Returns the location and average normal of faces from to_obj that are directly above or below from_obj
    ...
    for face in to_obj.data.polygons:  
        ...
        intersect = geometry.intersect_ray_tri(tVerts[0],tVerts[1],tVerts[2],Vector((0.0,0.0,1.0)),from_obj.location)
        if intersect:
            location = intersect
